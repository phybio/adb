...
run_transport_disconnects(atransport*  t)
{
    ...
    printf("%s: run_transport_disconnects\n", t->serial);
...
read_packet(int  fd, const char* name, apacket** ppacket)
{
    ...
    while(len > 0) {
        ...
        if(r > 0) {
            ...
            printf("%s: read_packet (fd=%d), error ret=%d errno=%d: %s\n", name, fd, r, errno, strerror(errno));
...
write_packet(int  fd, const char* name, apacket** ppacket)
{
    ...
    while(len > 0) {
        ...
        if(r > 0) {
            ...
            printf("%s: write_packet (fd=%d) error ret=%d errno=%d: %s\n", name, fd, r, errno, strerror(errno));
            if((r < 0) && (errno == EINTR)) continue;
            return -1;
        }
    }
...
static void transport_socket_events(int fd, unsigned events, void *_t)
{
    printf("function = %s, file = %s, line = %u \n", __FUNCTION__, __FILE__, __LINE__);
    atransport *t = _t;
    printf("transport_socket_events(fd=%d, events=%04x,...)\n", fd, events);
    if(events & FDE_READ){
        apacket *p = 0;
        if(read_packet(fd, t->serial, &p)){
            printf("%s: failed to read packet from transport socket on fd %d\n", t->serial, fd);
        } else {
...
static void *output_thread(void *_t)
{
    ...
    printf("%s: starting transport output thread on fd %d, SYNC online (%d)\n",
    ...
    printf("%s: data pump started\n", t->serial);
    for(;;) {
        p = get_apacket();

        if(t->read_from_remote(p, t) == 0){
            printf("%s: received remote packet, sending to transport\n",
              t->serial);
            if(write_packet(t->fd, t->serial, &p)){
                put_apacket(p);
            ...
            printf("%s: remote read failed for transport\n", t->serial);
            put_apacket(p);
            break;
        }
    }

    printf("%s: SYNC offline for transport\n", t->serial);
    p = get_apacket();
    p->msg.command = A_SYNC;
    p->msg.arg0 = 0;
    p->msg.arg1 = 0;
    ...
    if(write_packet(t->fd, t->serial, &p)) {
        put_apacket(p);
        printf("%s: failed to write SYNC apacket to transport", t->serial);
    }
...
static void *input_thread(void *_t)
{
    ...
    printf("%s: starting transport input thread, reading from fd %d\n",
       t->serial, t->fd);
        ...
        if(p->msg.command == A_SYNC){
            if(p->msg.arg0 == 0) {
                ...
                if(p->msg.arg1 == t->sync_token) {
                    printf("%s: transport SYNC online\n", t->serial);
                    active = 1;
                } else {
                    printf("%s: transport ignoring SYNC %d != %d\n",
                      t->serial, p->msg.arg1, t->sync_token);
                }
            }
        } else {
            if(active) {
                printf("%s: transport got packet, sending to remote\n", t->serial);
                t->write_to_remote(p, t);
            } else {
                printf("%s: transport ignoring packet while offline\n", t->serial);
            }
        }
...
transport_read_action(int  fd, struct tmsg*  m)
{
    ...
    while(len > 0) {
        ...
        if(r > 0) {
            ...
            if((r < 0) && (errno == EINTR)) continue;
            printf("transport_read_action: on fd %d, error %d: %s\n",
...
transport_write_action(int  fd, struct tmsg*  m)
{
    ...
    while(len > 0) {
        ...
        if(r > 0) {
            ...
            if((r < 0) && (errno == EINTR)) continue;
            printf("transport_write_action: on fd %d, error %d: %s\n",
              fd, errno, strerror(errno));
            return -1;
        }
    }
...
static void transport_registration_func(int _fd, unsigned ev, void *data)
{
    ...
    if(m.action == 0){
        printf("transport: %s removing and free'ing %d\n", t->serial, t->transport_socket);
    ...
    if (t->connection_state != CS_NOPERM) {
        ...
        printf("transport: %s (%d,%d) starting\n", t->serial, s[0], s[1]);

        t->transport_socket = s[0];
        t->fd = s[1];

        printf("function = %s, file = %s, line = %u, transport_registration_func, t->transport_socket = %d, transport_socket_events = %p.\n", __FUNCTION__, __FILE__, __LINE__, t->transport_socket, transport_socket_events);
...
void init_transport_registration(void)
{
    ...
    printf("function = %s, file = %s, line = %u, before tsp_reg_recv = %d, tsp_reg_func = %p.\n", __FUNCTION__, __FILE__, __LINE__, transport_registration_recv, transport_registration_func);
    fdevent_install(&transport_registration_fde,
                    transport_registration_recv,
                    transport_registration_func,
                    0);
    printf("function = %s, file = %s, line = %u, before fdevent_set \n", __FUNCTION__, __FILE__, __LINE__);
...
static void transport_unref_locked(atransport *t)
{
    ...
    t->ref_count--;
    if (t->ref_count == 0) {
        printf("transport: %s unref (kicking and closing)\n", t->serial);
        if (!t->kicked) {
            t->kicked = 1;
            t->kick(t);
        }
        ...
        printf("transport: %s unref (count=%d)\n", t->serial, t->ref_count);
    }
}
...
int readx(int fd, void *ptr, size_t len)
{
    ...
    while(len > 0) {
        ...
        if(r > 0) {
            ...
            if (r < 0) {
                printf("readx: fd=%d error %d: %s\n", fd, errno, strerror(errno));
                if (errno == EINTR)
                    continue;
            } else {
                printf("readx: fd=%d disconnected\n", fd);
            }
            return -1;
        }
    }
...
#if ADB_TRACE
    printf("readx: fd=%d wanted=%zu got=%zu\n", fd, len0, len0 - len);
    dump_hex( ptr, len0 );
#endif
    return 0;
}
...
int writex(int fd, const void *ptr, size_t len)
{
    ...
    printf("writex: fd=%d len=%d: ", fd, (int)len);
