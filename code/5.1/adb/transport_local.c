...
static void *client_socket_thread(void *x)
{
    ...
    printf("transport: client_socket_thread() starting\n");

    printf("function = %s, file = %s, line = %u, intheard client_socket_thread \n", __FUNCTION__, __FILE__, __LINE__);
...
static void *server_socket_thread(void * arg)
{
    ...
    printf("transport: server_socket_thread() starting\n");
    printf("function = %s, file = %s, line = %u, intheard server_socket_thread \n", __FUNCTION__, __FILE__, __LINE__);
    serverfd = -1;
    for(;;) {
        if(serverfd == -1) {
        ...
        alen = sizeof(addr);
        printf("server: trying to get new connection from %d\n", port);
        fd = adb_socket_accept(serverfd, &addr, &alen);
        if(fd >= 0) {
            printf("server: new connection on fd %d\n", fd);
            close_on_exec(fd);
            disable_tcp_nagle(fd);
            register_socket_transport(fd, "host", port, 1);
        }
    }
    printf("transport: server_socket_thread() exiting\n");
    return 0;
}
...
static const char _ok_resp[]    = "ok";
    ...
    printf("transport: qemu_socket_thread() starting\n");
    printf("function = %s, file = %s, line = %u, intheard qemu_socket_thread \n", __FUNCTION__, __FILE__, __LINE__);
    ...
    printf("transport: qemu_socket_thread() exiting\n");
    return 0;
}
#endif  // !ADB_HOST
...
void local_init(int port)
{
    ...
    if(HOST) {
        ...
        printf("function = %s, file = %s, line = %u, fun = client_socket_thread.\n", __FUNCTION__, __FILE__, __LINE__);
    } else {
#if ADB_HOST
        func = server_socket_thread;
        printf("function = %s, file = %s, line = %u, fun = server_socket_thread.\n", __FUNCTION__, __FILE__, __LINE__);
#else
        /* For the adbd daemon in the system image we need to distinguish
         * between the device, and the emulator. */
        char is_qemu[PROPERTY_VALUE_MAX];
        property_get("ro.kernel.qemu", is_qemu, "");
        printf("function = %s, file = %s, line = %u , property_get is_qemu = %s. \n", __FUNCTION__, __FILE__, __LINE__, is_qemu);
        if (!strcmp(is_qemu, "1")) {
            /* Running inside the emulator: use QEMUD pipe as the transport. */
            func = qemu_socket_thread;
	   printf("function = %s, file = %s, line = %u, fun = qemu_socket_thread.\n", __FUNCTION__, __FILE__, __LINE__);
        } else {
    ...
    if (HOST && local) {
        adb_mutex_lock( &local_transports_lock );
        {
            ...
            if (existing_transport != NULL) {
                printf("local transport for port %d already registered (%p)?\n",
